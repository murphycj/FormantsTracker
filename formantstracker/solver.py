...
class Solver:
    ...
    def init_test_loader(self):
        ...
        self.test_loader = DataLoader(
            test_dataset,
            batch_size=self.hp.test_batch_size,
            shuffle=False,
    ...
    def get_predicted_formants(self, out):
        # smooth output
        kernel = utils.get_smoothing_kernel(
            self.hp.gaussian_kernel_size, self.hp.gaussian_kernel_sigma
        ).to(out.device)
        out_smoothed = F.conv2d(
        ...
        return (
            (torch.max(out_smoothed, dim=3)[1]) * self.bin_resolution
    ...
    def write_predictions(self, predictions, lengths, fnames):
        for fname, length, pred in zip(fnames, lengths, predictions):
            ...
            pred_fname = os.path.join(
                self.hp.predictions_dir, fname[len(self.hp.test_dir) + 1 :]
            ).replace(".wav", ".pred")
            os.makedirs(os.path.dirname(pred_fname), exist_ok=True)
            with open(pred_fname, "w") as f:
                for i in range(length):
                    f.write(
                        f"{i/100}\t{int(pred[0][i])}\t{int(pred[1][i])}\t{int(pred[2][i])}\n"
                    )
